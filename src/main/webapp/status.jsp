...
<c:forEach var="item" items="${instances}">
    <tr>
      <td><c:out value="${item.instanceId }"/></td>
      <td><c:out value="${item.publicDnsName}"/></td>
      <td><c:out value="${item.publicIpAddress}"/></td>
      <td><c:out value="${item.placement.availabilityZone}"/></td>    
    </tr>
</c:forEach>
  </tbody>
</table>
